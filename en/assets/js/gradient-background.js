window.addEventListener("DOMContentLoaded", () => {
  ...
  // Only create canvas and initialize animation for non-mobile devices
  if (!isMobileDevice()) {
    ...
    // Handle window resize - check if device type changes
    window.addEventListener("resize", () => {
      if (isMobileDevice()) {
        // If changed to mobile view, remove canvas
        const existingCanvas = document.getElementById("gradient-bg-canvas");
        if (existingCanvas) {
          existingCanvas.remove();
        }
      } else {
        // If changed to desktop view and canvas doesn't exist, reinitialize
        if (!document.getElementById("gradient-bg-canvas")) {
          document.body.insertBefore(canvas, document.body.firstChild);
          resize();
          createParticles();
          updateGradientVisibility();
        } else {
          // Just resize if canvas already exists
          resize();
          createParticles();
          updateGradientVisibility();
        }
